...
#define MT9V03X_COF_IIC_DELAY   ( 1000 )                                         // 总钻风 IIC 延时
#define MT9V03X_COF_IIC_SCL     ( P11_1 )                                       // 总钻风 IIC-SCL 引脚
#define MT9V03X_COF_IIC_SDA     ( P11_2 )                                       // 总钻风 IIC-SDA 引脚

#define MT9V03X_PCLK_PIN        ( P08_1 )                                       // 总钻风 PCLK 触发信号引脚
#define MT9V03X_VSYNC_PIN       ( P08_0 )                                       // 总钻风 场中断  引脚

#define MT9V03X_DATA_PIN        ( P06_0 )                                       // 总钻风 数据 引脚 这里是 P06_0 ~ P06_6(CYT2BL3只有连续7个数据引脚)
//--------------------------------------------------------------------------------------------------
// 引脚配置
//--------------------------------------------------------------------------------------------------
...
// 摄像头默认参数配置 在此修改摄像头配置
//--------------------------------------------------------------------------------------------------
#define MT9V03X_W               ( 188 )                                         // 图像宽度     (可修改 不得超过188)  当分辨率为188 * 120时，可采集到至多200帧的图像(可能有噪点)
#define MT9V03X_H               ( 120 )                                         // 图像高度     (可修改 不得超过120)  
#define MT9V03X_IMAGE_SIZE      ( MT9V03X_W * MT9V03X_H )                       // 整体图像大小
...
#define MT9V03X_EXP_TIME_DEF    ( 500 )                                         // 曝光时间设置   摄像头收到后会自动计算出最大曝光时间，如果设置过大则设置为计算出来的最大曝光值
...
#define MT9V03X_UD_OFFSET_DEF   ( 0   )                                         // 图像上下偏移   正值 上偏移   负值 下偏移  行为120 240 480时无法设置偏移
                                                                                // 一一一一一一   摄像头收偏移数据后会自动计算最大偏移，如果超出则设置计算出来的最大偏移

#define MT9V03X_GAIN_DEF        ( 32  )                                         // 图像增益设置   范围 [16-64]  增益可以在曝光时间固定的情况下改变图像亮暗程度

#define MT9V03X_PCLK_MODE_DEF   ( 0   )                                         // 像素时钟模式   范围 [0-1]    默认：0 可选参数为：[0：不输出消隐信号,1：输出消隐信号]
                                                                                // 一一一一一一   通常都设置为0，如果使用CH32V307的DVP接口或STM32的DCMI接口采集需要设置为1
                                                                                // 一一一一一一   仅总钻风 MT9V034 V1.5 以及以上版本支持该命令
//--------------------------------------------------------------------------------------------------
// 摄像头默认参数配置 在此修改摄像头配置
//--------------------------------------------------------------------------------------------------
