...
float pid_calc_incremental(pid_t *pid, float kp, float ki, float kd,
                           float target, float actual);                                 // 增量式PID（已废弃，不推荐使用）
float pid_calc_position_speed(pid_t *pid, float kp, float ki, float kd,
                               float target, float actual);                             // 位置式PID（速度环用，支持自定义参数）
